...
class Player(Object):
  ...
  def update(self):
    ...
    if self.sleeping:
      if self.inventory['energy'] < constants.items['energy']['max']:
        action = 'sleep'
        action_applied = selected_action in ('noop', 'sleep')
      else:
        self.sleeping = False
    ...
    if action == 'noop':
      ...
      action_applied = self._move(action[len('move_'):])
    elif action == 'do' and obj:
      action_applied = self._do_object(obj)
    elif action == 'do':
      action_applied = self._do_material(target, material)
    elif action == 'sleep':
      if self.inventory['energy'] < constants.items['energy']['max']:
        self.sleeping = True
        action_applied = selected_action in ('noop', 'sleep')
    elif action.startswith('place_'):
      action_applied = self._place(action[len('place_'):], target, material)
    elif action.startswith('make_'):
      action_applied = self._make(action[len('make_'):])
    self.illegal_action = not action_applied
  ...
  def _move(self, direction):
    ...
    if self.world[self.pos][0] == 'lava':
      self.health = 0
    return moved or self.facing != previous_facing
