...
def generate_world(world, player, no_mobs=False):
  ...
  if world.count('diamond') == 0:
    candidates = []
    for x in range(world.area[0]):
      for y in range(world.area[1]):
        if world[x, y][0] in ('stone', 'iron', 'coal'):
          candidates.append((x, y))
    if candidates:
      world[candidates[world.random.randint(0, len(candidates))]] = 'diamond'

  for x in range(world.area[0]):
    for y in range(world.area[1]):
      _set_object(world, (x, y), player, tunnels, no_mobs=no_mobs)
...
def _set_object(world, pos, player, tunnels, no_mobs=False):
  x, y = pos
  uniform = world.random.uniform
  ...
  elif not no_mobs and dist > 10 and uniform() > 0.993:
    world.add(objects.Zombie(world, (x, y), player))
  elif not no_mobs and material == 'path' and tunnels[x, y] and uniform() > 0.95:
