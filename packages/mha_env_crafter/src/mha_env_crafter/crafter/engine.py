...
class World:
  ...
  def nearby(self, pos, distance):
    ...
    ids = set(self._mat_map[
        max(0, x - d): x + d + 1, max(0, y - d): y + d + 1].flatten().tolist())
...
class LocalView:
  ...
  def __init__(self,
    ...
    self._center = None
    self._daylight_effects = daylight_effects
    self._sleep_effects = sleep_effects
  ...
  def __call__(self, player, unit):
    ...
    if self._daylight_effects:
      canvas = self._light(canvas, self._world.daylight)
    if player.sleeping and self._sleep_effects:
